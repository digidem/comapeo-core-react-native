...
> **Shipped:** PR #15. iOS now consumes the same rolled-up backend as
> Android; `ios/nodejs-project/` is generated by `npm run backend:build`.
> Device support arrived with Phase 2 (xcframework migration). iOS
> simulator smoke test
> (`example/tests/ios/CoreManagerSmokeTest.swift`) verifies the
> embedded `ComapeoManager` instantiates end-to-end.
...
>   per-platform banner).
> - **Step 5 (JNI stdio drain fix)** stays deferred — separable from
>   packaging with a different failure mode. Tracked in issue #19.
