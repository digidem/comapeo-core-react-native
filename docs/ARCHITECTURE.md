...
| 6 | Android `ComapeoCoreService.onDestroy` `withTimeout` | 10 s | `nodeJSService.stopForTeardown()` graceful drain hangs | Catches `TimeoutCancellationException` → `Process.killProcess`. This is the only outer bound on the Android teardown drain — it has no internal timeout |
