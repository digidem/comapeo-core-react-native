...
export default function ObservationDetail() {
  ...
  const { data: o } = useSingleDocByDocId({
    projectId,
    docType: "observation",
    docId,
  });
  ...
  const initialTags = useMemo(
    () =>
      Object.fromEntries(
        Object.entries(o.tags ?? {}).map(([k, v]) => [k, String(v)]),
      ),
    [o.tags],
  );
      ...
      <Screen>
        ...
        <Section header={`Tags (${Object.keys(tags).length})`}>
          {Object.entries(tags).map(([k, v], i, arr) => (
            <FormField
              ...
              value={v}
              onChangeText={(next) =>
                setTags((prev) => ({ ...prev, [k]: next }))
              }
              isLast={i === arr.length - 1}
            />
          ))}
          {Object.keys(tags).length === 0 ? <Row title="No tags" /> : null}
        </Section>
        ...
        <Section header="Common (read-only)">
          <FormField
            ...
            readOnly
            right={<ShortId id={o.docId} size="xs" />}
          />
          <FormField
            label="versionId"
            value={shortId(o.versionId)}
            readOnly
            right={<ShortId id={o.versionId} size="xs" />}
          />
          <FormField
            label="originalVersionId"
            value={shortId(o.originalVersionId)}
            readOnly
            right={<ShortId id={o.originalVersionId} size="xs" />}
          />
          <FormField label="schemaName" value={o.schemaName} readOnly />
          <FormField
            label="createdAt"
            value={fmtDateTime(o.createdAt)}
            readOnly
          />
          <FormField
            label="updatedAt"
            value={fmtDateTime(o.updatedAt)}
            readOnly
          />
          <FormField
            label="deleted"
            value={String(o.deleted)}
            readOnly
            isLast
          />
        </Section>
