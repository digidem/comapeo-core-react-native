...
export default function PresetDetail() {
  ...
  const { data: preset } = useSingleDocByDocId({
    projectId,
    docType: "preset",
    docId,
  });
      ...
      <Screen>
        ...
        <Section header="Common (read-only)">
          <FormField
            ...
            readOnly
            right={<ShortId id={preset.docId} size="xs" />}
          />
          <FormField
            label="versionId"
            value={shortId(preset.versionId)}
            readOnly
            right={<ShortId id={preset.versionId} size="xs" />}
          />
          <FormField label="schemaName" value={preset.schemaName} readOnly />
          <FormField
            label="createdAt"
            value={fmtDateTime(preset.createdAt)}
            readOnly
            isLast
          />
        </Section>
