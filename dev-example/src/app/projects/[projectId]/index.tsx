    ...
    <>
      <Stack.Screen
        options={{
          ...
          headerRight: () =>
            Platform.OS === 'ios' ? (
              <HeaderButton label="Settings" onPress={() => router.push(`/projects/${projectId}/settings`)} />
            ) : undefined,
        }}
      />
      <Screen>
        <View style={styles.idRow}>
          <ShortId id={projectId} label="projectId" size="md" />
        </View>
        ...
        <Section header="Project">
          <Row title="Settings" onPress={() => router.push(`/projects/${projectId}/settings`)} />
          <Row
            isLast
            title={<Text style={{ color: T.danger, fontSize: 16 }}>Leave project</Text>}
...
function peerCount(sync: { remoteDeviceSyncState?: Record<string, unknown> } | null): number {
