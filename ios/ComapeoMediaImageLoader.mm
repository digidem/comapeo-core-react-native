...
// comapeo://...". This file plugs into that lookup so RN's built-in
// <Image> can fetch our `comapeo://media/...` URLs.
//
// Implemented in Obj-C++ (.mm) so:
//   - RCT_EXPORT_MODULE() is available (RN's autolinking glue needs an
//     Obj-C class on the +load runtime hook).
//   - We can call into the shared Swift `MediaFetcher` (same UDS-fetch
//     logic that backs the streaming URLProtocol).
//
...
// existing http(s) image path — same peak memory shape. See
// MediaFetcher.swift for the streaming variant used by non-RN-Image
// consumers.
...
// EXBaseAppDelegateSubscriber". `@import ExpoModulesCore;` would fix it
// but is rejected by this Pod's compile flags ("use of '@import' when
// C++ modules are disabled"). A forward declaration of just the symbols
// we call is the smallest surface that satisfies the compiler; linking
// is fine because Swift's @objc emits the methods at these runtime names.
